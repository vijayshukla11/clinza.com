...
import { INITIAL_PRODUCTS, INITIAL_CATEGORIES, INITIAL_BLOGS } from "./src/data";
...
app.get("/sitemap.xml", (req, res) => {
  ...
  staticUrls.forEach((urlPath) => {
    xml += `  <url>\n`;
    xml += `    <loc>${host}${urlPath}</loc>\n`;
    xml += `    <lastmod>${now}</lastmod>\n`;
    xml += `    <changefreq>${urlPath === "" ? "daily" : "weekly"}</changefreq>\n`;
  ...
  INITIAL_CATEGORIES.forEach((cat) => {
    xml += `  <url>\n`;
    xml += `    <loc>${host}/collections/${cat.slug}</loc>\n`;
    xml += `    <lastmod>${now}</lastmod>\n`;
  ...
  // Product URLs (filter active)
  INITIAL_PRODUCTS.filter(p => (p as any).isActive !== false).forEach((prod) => {
    xml += `  <url>\n`;
    xml += `    <loc>${host}/product/${prod.slug || prod.id}</loc>\n`;
    xml += `    <lastmod>${now}</lastmod>\n`;
  ...
  INITIAL_BLOGS.forEach((blog) => {
    xml += `  <url>\n`;
    xml += `    <loc>${host}/blog/${blog.slug}</loc>\n`;
    xml += `    <lastmod>${blog.publishedAt ? blog.publishedAt.split("T")[0] : now}</lastmod>\n`;
...
// 3. Google Merchant Center XML Feed for Google Shopping
app.get("/api/feeds/merchant.xml", (req, res) => {
  const host = process.env.PUBLIC_DOMAIN || "https://www.clinza.in";

  let xml = `<?xml version="1.0" encoding="UTF-8"?>\n`;
  xml += `<rss version="2.0" xmlns:g="http://base.google.com/ns/1.0">\n`;
  xml += `  <channel>\n`;
  xml += `    <title>CLINZA Product Catalog - Google Shopping</title>\n`;
  xml += `    <link>${host}</link>\n`;
  xml += `    <description>CLINZA Premium Luxury Organic Cotton and Linen Apparel</description>\n`;

  INITIAL_PRODUCTS.filter(p => (p as any).isActive !== false).forEach((prod) => {
    const isOut = prod.stockStatus === "Out of Stock";
    const brandName = prod.brand || "CLINZA";
    const prodUrl = `${host}/product/${prod.slug || prod.id}`;
    const imgUrl = (prod.images && prod.images[0]) || (prod as any).image || "";
    const cleanDesc = (prod.description || prod.shortDescription || prod.name).replace(/<[^>]*>?/gm, "");

    xml += `    <item>\n`;
    xml += `      <g:id>${xmlEscape(prod.id)}</g:id>\n`;
    xml += `      <g:title>${xmlEscape(prod.name)}</g:title>\n`;
    xml += `      <g:description>${xmlEscape(cleanDesc)}</g:description>\n`;
    xml += `      <g:link>${xmlEscape(prodUrl)}</g:link>\n`;
    xml += `      <g:image_link>${xmlEscape(imgUrl)}</g:image_link>\n`;
    xml += `      <g:condition>new</g:condition>\n`;
    xml += `      <g:availability>${isOut ? "out_of_stock" : "in_stock"}</g:availability>\n`;
    xml += `      <g:price>${prod.price} INR</g:price>\n`;
    xml += `      <g:brand>${xmlEscape(brandName)}</g:brand>\n`;
    xml += `      <g:google_product_category>Apparel &amp; Accessories &gt; Clothing</g:google_product_category>\n`;
    if (prod.sku) {
      xml += `      <g:mpn>${xmlEscape(prod.sku)}</g:mpn>\n`;
    }
    if (prod.category) {
      xml += `      <g:product_type>${xmlEscape(prod.category)}</g:product_type>\n`;
    }
    xml += `    </item>\n`;
  });
