...
// Helper to parse seo_keywords into consistent string format
...
export async function syncProductsFromCloud(): Promise<Product[]> {
  try {
    ...
    // Map DB snake_case or JSON to typescript camelCase structure cleanly
    return (data || []).map((row: any) => {
      ...
      return {
        ...
        stockQuantity: Number(row.stock_quantity ?? row.stockQuantity ?? 50),
        ...
        fabricCare: row.fabric_care ?? row.fabricCare ?? "",
        ...
        trendingRank: row.trending_rank !== undefined && row.trending_rank !== null ? Number(row.trending_rank) : undefined,
...
export async function getGuestOrderFromCloud(orderId: string, contactInfo: string): Promise<Order | null> {
  try {
    if (!orderId || !contactInfo) return null;

    const { data, error } = await supabase.rpc("get_guest_order_by_id", {
      p_order_id: orderId.trim(),
      p_contact: contactInfo.trim()
    });

    if (error) {
      console.warn("Supabase guest order tracking RPC error:", error.message);
      return null;
    }
    ...
    const row = data[0];
    const createdDate = row.out_created_at 
    ...
    return {
      ...
      customer: {
        ...
        phone: "******",
        email: "******",
        address: "Protected for Security",
        city: row.out_destination_city || "",
        state: row.out_destination_state || "",
        country: "India",
      ...
      date: createdDate
    } as Order & { date?: string };
  } catch (err) {
    console.error("Order tracking RPC fetch error:", err);
    return null;
  }
}

export async function getSingleOrderFromCloud(orderId: string, contactInfo?: string): Promise<Order | null> {
  if (contactInfo) {
    return getGuestOrderFromCloud(orderId, contactInfo);
  }
