...
export default function ShopAllCollectionsPage({
  ...
  const getProductCount = (collectionIdOrSlug: string) => {
    if (!collectionIdOrSlug) return 0;
    const colObj = collections.find(c => c.slug === collectionIdOrSlug || c.id === collectionIdOrSlug);
    return allProducts.filter(p => isProductInCollection(p, collectionIdOrSlug, colObj)).length;
  };
  ...
  useEffect(() => {
    ...
    if (selectedCategory !== "all") {
      result = result.filter(p => isProductInCollection(p, selectedCategory));
    }

    // Filter by Shopify collection definitions
    if (selectedCollection !== "all") {
      const colObj = collections.find(c => c.slug === selectedCollection || c.id === selectedCollection);
      result = result.filter(p => isProductInCollection(p, selectedCollection, colObj));
    }
