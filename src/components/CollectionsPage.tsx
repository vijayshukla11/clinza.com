...
export default function CollectionsPage({ setRoute }: CollectionsPageProps) {
  ...
  const getProductCount = (collectionSlugOrId: string): number => {
    ...
    if (!normalized) return allProducts.length;

    let count = 0;
    if (normalized === "premium-linen" || normalized === "linen-shirts") {
      count = allProducts.filter(
        (p) => p.category?.toLowerCase().includes("linen") || p.description?.toLowerCase().includes("linen") || p.name?.toLowerCase().includes("linen")
      ).length;
      ...
      count = allProducts.filter(
        (p) => p.collection?.toLowerCase().includes("shirt") || p.category?.toLowerCase().includes("shirt") || p.name?.toLowerCase().includes("shirt")
      ).length;
    } else if (normalized === "jeans") {
      count = allProducts.filter(
        (p) => p.collection?.toLowerCase().includes("jean") || p.category?.toLowerCase().includes("jean") || p.name?.toLowerCase().includes("jean") || p.category?.toLowerCase().includes("denim")
      ).length;
    } else if (normalized === "pants" || normalized === "trousers") {
      count = allProducts.filter(
        (p) => p.collection?.toLowerCase().includes("pant") || p.collection?.toLowerCase().includes("trouser") || p.category?.toLowerCase().includes("pant") || p.category?.toLowerCase().includes("trouser")
      ).length;
    } else {
      const cleanSlug = normalized.replace(/^(col-|cat-|collection-)/, "").trim();
      count = allProducts.filter(
        (p) =>
          (p.collection && (p.collection.toLowerCase().includes(normalized) || p.collection.toLowerCase().includes(cleanSlug))) ||
          (p.category && (p.category.toLowerCase().includes(normalized) || p.category.toLowerCase().includes(cleanSlug))) ||
          (p.description && (p.description.toLowerCase().includes(normalized) || p.description.toLowerCase().includes(cleanSlug))) ||
          (p.name && (p.name.toLowerCase().includes(normalized) || p.name.toLowerCase().includes(cleanSlug)))
      ).length;
    }
    return count > 0 ? count : (allProducts.length > 0 ? allProducts.length : 12);
  };
