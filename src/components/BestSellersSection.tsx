...
function isEqualProducts(a: Product[], b: Product[]): boolean {
  if (a === b) return true;
  ...
  for (let i = 0; i < a.length; i++) {
    if (
      a[i].id !== b[i].id ||
      a[i].price !== b[i].price ||
      a[i].originalPrice !== b[i].originalPrice ||
      a[i].stockStatus !== b[i].stockStatus ||
      a[i].images?.[0] !== b[i].images?.[0]
    ) {
      return false;
    }
  }
...
function getInitialBestSellers(limit?: number): Product[] {
    ...
    try {
      ...
      if (cachedStr) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          const sliced = limit && limit > 0 ? parsed.slice(0, limit) : parsed.slice(0, 8);
          return sliced;
        }
      }
    } catch {}
...
export default function BestSellersSection({
  ...
  const initialProducts = getInitialBestSellers(limit);
  ...
  useEffect(() => {
    ...
    async function loadData(isInitial = false) {
      try {
        if ((isInitial && products.length === 0) || products.length === 0) {
          setLoading(true);
        }
        const data = await ProductsService.getHomepageBestSellers();
        if (isMounted && data) {
          const sliced = limit && limit > 0 ? data.slice(0, limit) : data;
          setProducts(prev => {
            if (isEqualProducts(prev, sliced)) return prev;
            return sliced;
          });
        }
      } catch (err) {
    ...
    window.addEventListener("clinza_products_updated", handleUpdate);
    window.addEventListener("clinza_homepage_updated", handleUpdate);
    return () => {
      isMounted = false;
      window.removeEventListener("clinza_products_updated", handleUpdate);
      ...
      <div className="max-w-[1440px] mx-auto">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="relative aspect-[4/5] w-full rounded-[10px] overflow-hidden bg-[#F5F5F3]">
                      <img
                        src={(Array.isArray(product.images) && product.images[0]) || "https://images.unsplash.com/photo-1521572267360-ee0c2909d518?auto=format&fit=crop&w=800&q=80"}
                        alt={product.name}
                        ...
                        <div className="flex items-center justify-between mb-1 text-[11px]">
                          <div className="flex items-center gap-1 text-amber-500 font-semibold">
                            ...
                            <span className="text-gray-400 font-normal">
                              ({(Array.isArray(product.reviews) ? product.reviews.length : 28)})
                            </span>
                          </div>
        ...
        <div className="mt-10 sm:mt-14 text-center">
          <button
            ...
            onClick={() => {
              setRoute(ctaUrl || "collections/best-sellers");
              window.scrollTo({ top: 0, behavior: "smooth" });
            }}
