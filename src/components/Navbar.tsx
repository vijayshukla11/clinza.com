...
import { getSearchHistory, addSearchHistory, clearSearchHistory, getProducts, calculateComboPromotion, calculateCartTotals } from "../utils";
  ...
  useEffect(() => {
    ...
    async function loadNavCollections() {
      try {
        ...
        if (isMounted) {
          const list: CollectionItem[] = cols ? cols.filter((c) => c.isActive !== false) : [];
          setNavCollections(list.sort((a, b) => (a.displayOrder || 0) - (b.displayOrder || 0)));
        }
      } catch (e) {
              ...
              <div className="col-span-3 space-y-4">
                ...
                <ul className="space-y-3">
                  {[
                    ...
                    { name: "Best Sellers", route: "collections/best-sellers" },
        ...
        <div className="fixed inset-0 z-50 overflow-hidden">
          ...
          <div className="fixed inset-y-0 right-0 max-w-full flex pl-10">
            <div className="w-full max-w-md bg-white shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              {/* Combo Tier Promotion Progress */}
              {(() => {
                const promo = calculateComboPromotion(cart);
                return (
                  <div className="bg-orange-50/60 px-6 py-3 border-b border-orange-100/60 text-left">
                    <div className="flex items-center justify-between text-[11px] font-bold text-gray-900 uppercase tracking-wider mb-1">
                      <span>{promo.tierBadge || "CLINZA COMBO PROMOTION"}</span>
                      <span className="font-mono text-orange-600 font-black">{promo.comboCount} COMBO{promo.comboCount === 1 ? "" : "S"}</span>
                    </div>
                    <p className="text-[11px] font-medium text-gray-700 font-sans">
                    ...
                    <div key={`${item.product.id}-${item.selectedColor}-${item.selectedSize}-${idx}`} className="flex gap-4 border-b border-zinc-100 pb-6">
                      ...
                      <div className="flex-1 flex flex-col justify-between">
                        ...
                        <div className="flex justify-between items-center mt-3">
                          {item.isFreeItem ? (
                            <span className="text-[10px] font-bold text-emerald-700 font-mono uppercase bg-emerald-100 px-2 py-0.5 rounded">
                              1 Free Gift
                            </span>
                          ) : (
