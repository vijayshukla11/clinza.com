...
export default function BlogSystem({
  ...
  React.useEffect(() => {
    if (activeBlogSlug && !activeBlog) {
    return (
      <NotFoundPage navigate={navigate} handleOldRouteTrigger={setRoute} />
    );
  }
          ...
          <div className="prose prose-orange max-w-none text-gray-800 text-sm sm:text-base font-sans leading-relaxed sm:leading-loose font-normal space-y-5 sm:space-y-6">
            {activeBlog.content ? (
              activeBlog.content.split("\n\n").map((para, pIdx) => (
                <p key={pIdx} className="leading-relaxed sm:leading-loose whitespace-pre-line">
                  {para}
                </p>
              ))
            ) : (
