...
export default function ProductsTab({ productList, onSaveProduct, onDeleteProduct }: ProductsTabProps) {
  ...
  const handleOpenEditor = (prod: Product | null) => {
    if (prod) {
      ...
      const hoverImg = prod.hoverImage || (prod as any).hoverUrl || (prod as any).hover_image || (prod as any).hover_url || prod.images?.[1] || "";
      ...
      setForm({
        ...
        canonicalUrl: (prod as any).canonicalUrl || `https://www.clinza.in/product/${prod.slug}`,
      ...
      setForm({
        ...
        hoverImage: "https://vdtbquxxpikniarmjpai.supabase.co/storage/v1/object/public/products/slider/combo%20collection%20linen%20set.png",
        thumbnailImage: "https://vdtbquxxpikniarmjpai.supabase.co/storage/v1/object/public/products/slider/combo%20collection%20linen%20set.png",
        galleryImagesStr: "https://vdtbquxxpikniarmjpai.supabase.co/storage/v1/object/public/products/slider/combo%20collection%20linen%20set.png",
        view360Str: "",
        videoUrl: "",
  ...
  const handleSlugSync = (nameVal: string) => {
    if (editorMode === "create") {
      ...
      setForm(f => ({ 
        ...
        name: nameVal, 
        slug: generatedSlug,
        canonicalUrl: `https://www.clinza.in/product/${generatedSlug}`,
  ...
  const handleDistributeStock = () => {
    ...
    const currentVariants = editingProduct?.variants && editingProduct.variants.length > 0
      ? editingProduct.variants
      : generateVariantsForProduct(colorsArr, form.sizesList, form.sku, form.price, form.originalPrice, finalImages, form.stockQuantity);

    const distributed = distributeStockAcrossVariants(currentVariants, form.stockQuantity);
    if (editingProduct) {
      setEditingProduct({
        ...editingProduct,
    ...
    alert(`Allocated parent stock (${form.stockQuantity} units) evenly across ${distributed.length} variants.`);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    let finalImages = [form.mainImage || form.thumbnailImage].filter(Boolean);
    if (form.hoverImage) finalImages.push(form.hoverImage);
    finalImages.push(...galleryArr);
    finalImages = finalImages.filter(Boolean);

    // Safety guard: If editing an existing product and finalImages is empty, preserve existing product images
    if (finalImages.length === 0 && editingProduct?.images && editingProduct.images.length > 0) {
    ...
    // Deduplicate slug using ensureUniqueSlug (e.g. product, product-2, product-3...)
    const finalSlug = ensureUniqueSlug(form.slug, form.id, productList);
    const finalCanonicalUrl = form.canonicalUrl.trim() || `https://www.clinza.in/product/${finalSlug}`;

    // Auto JSON-LD build without fabricated dates or fake ratings
    const jsonLdObj: any = {
      "@context": "https://schema.org/",
      "@type": "Product",
      "name": form.name.trim(),
      "image": finalImages,
      "description": form.shortDescription || form.longDescription || form.name.trim(),
      "offers": {
        "@type": "Offer",
    ...
    if (form.sku.trim()) jsonLdObj.sku = form.sku.trim();
    if (form.barcode.trim()) jsonLdObj.mpn = form.barcode.trim();

    const generatedLd = form.jsonLdSchema.trim() || JSON.stringify(jsonLdObj, null, 2);

    // Preserve custom variant pricing or sync base values if variants inherited them
    const oldBasePrice = editingProduct?.price ?? 0;
    const oldOriginalPrice = editingProduct?.originalPrice ?? 0;

    let updatedVariants = editingProduct?.variants;
    if (updatedVariants && updatedVariants.length > 0) {
      updatedVariants = updatedVariants.map((v) => {
        // If variant price was undefined/null or matched old base product price, it inherited base price -> sync
        const isInheritedPrice = v.price === undefined || v.price === null || v.price === oldBasePrice;
        const isInheritedOriginalPrice = v.originalPrice === undefined || v.originalPrice === null || v.originalPrice === oldOriginalPrice;

        return {
          ...v,
          price: isInheritedPrice ? form.price : v.price,
          originalPrice: isInheritedOriginalPrice ? form.originalPrice : v.originalPrice,
        };
      });
    } else if (colorsArr.length > 0 || form.sizesList.length > 0) {
      updatedVariants = generateVariantsForProduct(
        colorsArr,
        form.sizesList,
        form.sku,
        form.price,
    ...
    const completeProduct: Product = {
      ...
      images: finalImages,
      hoverImage: form.hoverImage.trim() || finalImages[1] || finalImages[0] || "",
      ...
      shortDescription: form.shortDescription.trim() || form.longDescription.trim().slice(0, 160),
      fabric: form.fabric,
      fit: form.fitType,
      ...
      {editorMode === "list" ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-white border rounded-2xl p-6 shadow-sm">
          ...
          <div className="flex border-b border-zinc-200 gap-2 pb-1">
            ...
            <button
              type="button"
              onClick={() => setEditFormTab("reviews")}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="p-4 bg-zinc-50 rounded-xl space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="px-3 py-1 bg-orange-600 hover:bg-orange-700 text-white rounded-lg text-[10px] font-bold uppercase tracking-wider transition cursor-pointer"
                  >
                    Distribute Stock ({form.stockQuantity} Total)
                  </button>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                ...
                {editingProduct?.variants && editingProduct.variants.length > 0 && (
                  <div className="mt-3 pt-3 border-t border-zinc-200/80 space-y-2">
                    <div className="text-[10px] font-bold uppercase tracking-wider text-zinc-500 flex justify-between">
                      <span>Variant Matrix ({editingProduct.variants.length})</span>
                      <span>Adjust Stock Per Variant</span>
                    </div>
                    <div className="max-h-40 overflow-y-auto space-y-1.5 pr-1">
                      {editingProduct.variants.map((v, vIdx) => (
                        <div key={v.id || vIdx} className="flex items-center justify-between text-xs bg-white p-2 border rounded-lg">
                          <div className="flex items-center gap-2 font-mono text-[11px]">
                            <span className="w-3 h-3 rounded-full border border-zinc-300" style={{ backgroundColor: v.colorHex || "#333" }} />
                            <span className="font-bold text-zinc-800">{v.colorName} / {v.size}</span>
                            <span className="text-zinc-400 text-[10px]">({v.sku})</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] text-zinc-500 font-mono">Qty:</span>
                            <input
                              type="number"
                              min={0}
                              ...
                              onChange={(e) => {
                                ...
                                updatedVars[vIdx] = {
                                  ...
                                  status: newQty <= 0 ? "Out of Stock" : (newQty <= 5 ? "Low Stock" : "In Stock")
                                };
                                setEditingProduct({ ...editingProduct, variants: updatedVars });
                              }}
                              className="w-16 border rounded px-2 py-0.5 text-xs text-right font-mono focus:ring-1 focus:ring-orange-500 bg-zinc-50"
                            />
                          </div>
                        </div>
                      ))}
            ...
            <div className="lg:col-span-4 space-y-6">
              ...
              <div className="p-4 border bg-zinc-50/60 rounded-xl space-y-4 text-xs">
                ...
                {/* COLLECTION SEARCHABLE DROPDOWN */}
                <div className="relative">
                  <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Product Collection</label>
                  <div 
                    onClick={() => {
                      setShowColDropdown(!showColDropdown);
                      setShowCatDropdown(false);
                    }}
                    className="w-full border rounded-lg p-2.5 bg-white font-semibold text-zinc-800 flex justify-between items-center cursor-pointer hover:border-black transition-colors text-xs"
                  >
                    <span>
                      {collectionsList.find(c => c.slug === form.collection)?.name || form.collection || "Select Collection..."}
                    </span>
                    <span className="text-zinc-400 text-[9px]">▼</span>
                  </div>

                  {showColDropdown && (
                    <div className="absolute z-50 mt-1 w-full bg-white border border-zinc-200 shadow-xl rounded-lg p-2 space-y-2 max-h-60 overflow-y-auto">
                      <div className="relative">
                        ...
                        <input
                          ...
                          className="w-full border rounded-lg pl-8 pr-3 py-1.5 text-xs bg-zinc-50 focus:outline-none focus:ring-1 focus:ring-orange-500"
                        />
                      </div>
                      <div className="space-y-0.5">
                        {collectionsList
                          .filter(c => c.name.toLowerCase().includes(colSearch.toLowerCase()) || c.slug.toLowerCase().includes(colSearch.toLowerCase()))
                          .map((col) => (
                            <button
                              key={col.id}
                              type="button"
                              onClick={(e) => {
                                e.stopPropagation();
                                setForm(f => ({ ...f, collection: col.slug, category: col.name }));
                                setShowColDropdown(false);
                                setColSearch("");
                              }}
                              className={`w-full text-left px-3 py-2 rounded-md text-xs font-medium transition-colors flex justify-between items-center ${
                            ...
                            >
                              <span>{col.name}</span>
                              {form.collection === col.slug && <Check className="h-3.5 w-3.5 text-white" />}
                            </button>
                          ))
                        }
                      </div>
                ...
                <div className="relative">
                  <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Product Category</label>
                  <div 
                    onClick={() => {
                  ...
                  >
                    <span>
                      {categoriesList.find(c => c.name.toLowerCase() === form.category.toLowerCase() || c.slug === form.category)?.name || form.category || "Select Category..."}
                    </span>
                    <span className="text-zinc-400 text-[9px]">▼</span>
                  </div>
                      ...
                      <div className="space-y-0.5">
                        {categoriesList
                          ...
                          .map((cat) => (
                            ...
                            >
                              <span>{cat.name}</span>
                              {form.category.toLowerCase() === cat.name.toLowerCase() && <Check className="h-3.5 w-3.5 text-white" />}
                            </button>
                          ))
                        }
                      </div>
