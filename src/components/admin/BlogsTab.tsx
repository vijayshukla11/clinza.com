...
export default function BlogsTab({ blogList, onSaveBlog, onDeleteBlog }: BlogsTabProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const completeBlog: BlogPost = {
      ...
      coverImage: form.coverImage.trim() || "https://vdtbquxxpikniarmjpai.supabase.co/storage/v1/object/public/products/slider/combo%20collection%20linen%20set.png",
    ...
    <div id="blog-editorial-cms" className="space-y-6 text-left animate-fade-in">
      {editorMode === "list" ? (
        ...
        <form onSubmit={handleSubmit} className="bg-white border rounded-2xl p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-sans">
            <div>
              <label className="block text-[10px] font-bold uppercase text-zinc-500 mb-1">Publishing Status</label>
              <select
                value={form.state}
